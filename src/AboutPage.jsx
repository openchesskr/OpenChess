...
const VERSION_HISTORY = [
  {
    ...
    sections: [
      { cat: "feature", items: [
        ...
        "퍼즐 만들기 기능이 분석 탭 보드 편집기 대신 퍼즐 탭으로 옮겨졌어요 — '퍼즐 풀기/퍼즐 만들기' 선택 박스로 전환하면, PGN·FEN 코드 입력 → 퍼즐 유형 선택 → 생성된 라인 미리보기 순서로 진행해 대국 없이도 직접 퍼즐을 만들 수 있어요. PGN을 직접 타이핑하는 대신, 유산 만들기처럼 내 chess.com 대국 중 하나를 골라 그 기보를 바로 채워 넣을 수도 있고, 마지막 수 대신 유형에 실제로 해당하는 수 목록에서 직접 고를 수도 있어요. 로그인하지 않은 상태에서는 이 기능을 쓸 수 없어요.",
      ] },
      { cat: "ui", items: [
        "/about 페이지 첫 화면(히어로)이 더 세련된 디자인으로 새 단장했어요 — '기능 둘러보기' 버튼과, 학습·도감·퍼즐/티어/정확도 체계 소개로 바로 이동하는 칩 버튼이 추가됐어요.",
        "퍼즐 카드가 세로로 더 넉넉해지고, 미리보기 체스보드가 카드 폭에 맞춰 정사각형으로 훨씬 크게 보여요. 제목 길이와 무관하게 카드 비율이 항상 일정해요.",
        "퍼즐 탭 정렬 UI가 정리됐어요 — 검색창 옆 깔때기 아이콘 버튼 하나로 정렬 기준(추천순·최신순·레이팅순)뿐 아니라 '내가 만든 퍼즐'·'풀고 있는 중' 빠른 필터, 테마(기물 희생하기·우위 점하기·실수 응징하기), 전체/PGN/FEN 구분, 해결 완료 숨기기까지 한 드롭다운에서 골라요. 이 드롭다운은 개발자의 키워드 편집 박스와 같은 어두운 카드 레이아웃으로 그려져요.",
        "PGN·FEN 정보가 있는 퍼즐은 카드의 레이팅 배지 왼쪽에 PGN·FEN 표시가 떠요. 레이팅 배지를 누르면 이 퍼즐의 레이팅·내 레이팅(차이)·체감 난이도를 색으로 강조해 보여주는 말풍선이 떠요 — 목록 카드뿐 아니라 실제로 퍼즐을 푸는 화면에도 똑같이 적용돼요.",
      ] },
      { cat: "fix", items: [
