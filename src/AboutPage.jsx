...
const VERSION_HISTORY = [
  {
    ...
    summary: "'상점' 탭이 '플레이' 탭으로 개편됐어요 — 분석 탭의 PLAY 버튼과 똑같은 화면으로 곧장 들어가고, 그 화면을 아래로 내리면 기존 상점(스킨) 화면이 그대로 이어져요. 플레이 페이지 맨 위에는 '일반/스페셜' 토글이 생겼고, 스페셜에서는 실시간 대전 미니게임 두 가지를 한 줄씩 만나볼 수 있어요 — 무작위 좌표가 뜨면 상대보다 먼저 클릭하는 '좌표 인지 게임', 나이트로 목표 칸까지 상대보다 먼저 도달하는 5전 3선승 '나이트 경주'예요. 플레이 탭도 이제 다른 탭처럼 상단 헤더·하단 탭바가 함께 보여요(예전엔 화면 전체를 덮는 별도 화면이었어요) — 대국 중 다른 탭을 둘러봐도 진행 중이던 대국은 끊기지 않아요. 하단 탭 순서도 분석·플레이·퍼즐·학습·도감·설정으로 정리했고, 도감 탭 오프닝 모식도의 흰 영역을 데스크톱에서 조금 더 넓혔어요. 분석 탭 수 블록의 리체스 채택률(%)이 반올림 때문에 부정확하게 보이던 것도 고쳐, 이제 소수점 둘째 자리까지 실제 값에 더 가깝게 보여요. 모바일에서 분석 탭 엔진 라인(상위 3줄)이 안 뜨던 문제도 고쳤어요. 그리고 퍼즐 삭제, FEN 퍼즐 이름 변경이 안 되던 버그를 근본 원인부터 고쳤어요 — 알고 보니 서버가 '완료했다'는 응답을 빈 몸으로 보내는 요청 전부를, 앱이 그 빈 응답을 읽으려다 실패해서 무조건 '실패'로 잘못 판단하고 있었어요. 같은 이유로 조용히 안 되고 있던 계정 탈퇴, 채팅 메시지 수정·대화 지우기, 실시간 대국 대기열/초대 취소 같은 기능들과, 내가 만든 퍼즐이 오늘의 퍼즐로 뽑혔는데도 알림이 안 오던 문제도 이번에 함께 고쳐졌어요.",
    mascot: {
