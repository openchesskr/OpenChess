...
const VERSION_HISTORY = [
  {
    ...
    sections: [
      { cat: "feature", items: [
        ...
        "퍼즐 만들기 기능이 분석 탭 보드 편집기 대신 퍼즐 탭으로 옮겨졌어요 — '퍼즐 풀기/퍼즐 만들기' 선택 박스로 전환하면, PGN·FEN 코드 입력 → 퍼즐 유형 선택 → 생성된 라인 미리보기 순서로 진행해 대국 없이도 직접 퍼즐을 만들 수 있어요. PGN을 직접 타이핑하는 대신, 유산 만들기처럼 내 chess.com 대국 중 하나를 골라 그 기보를 바로 채워 넣을 수도 있어요.",
      ] },
      { cat: "ui", items: [
        ...
        "퍼즐 탭 정렬 UI가 정리됐어요 — 검색창 옆 깔때기 아이콘 버튼 하나로 정렬 기준(추천순·최신순·레이팅순)뿐 아니라 '내가 만든 퍼즐'·'풀고 있는 중' 빠른 필터, 테마(기물 희생하기·우위 점하기·실수 응징하기), 해결 완료 숨기기까지 한 드롭다운에서 골라요. 이 드롭다운은 개발자의 키워드 편집 박스와 같은 어두운 카드 레이아웃으로 그려져요.",
      ] },
      { cat: "fix", items: [
