...
const VERSION_HISTORY = [
  {
    ...
    summary: "chess.com 대국이 끝나면 요약 알림이 떠요 — 그리고 도감 오프닝 트리가 훨씬 빠르고 깔끔해졌어요.",
    mascot: {
      intro: { char: "kokoa", expr: "happy", name: "KOKOA 코치", align: "left", text: "방금 둔 대국, 전적이 어떻게 바뀌었는지 볼까요?" },
      outro: { char: "milku", expr: "wink", name: "MILKU 코치", align: "right", text: "체크메이트 이펙트, 직접 보고 싶지 않아요?" },
    },
    highlight: { kind: "icon", Icon: Sparkles, color: T.brassHi, label: "대국 요약 알림 · 새 도감 트리 · 전적 칩 · 대국 종료 이펙트 · 미니게임 드래그 무브" },
    sections: [
      { cat: "feature", items: [
        "chess.com 대국이 끝날 때마다 대국 요약 알림이 떠요 — 결과·상대·오프닝, 그리고 이 대국으로 그 오프닝 전적과 승률이 어떻게 바뀌었는지 보여주고, 검색·리뷰 버튼으로 바로 복기할 수 있어요. 접속하지 않은 사이 둔 대국은 다음 접속 때 차례로 보여줘요.",
        "대국이 끝나면 체크메이트(승자·패자), 스테일메이트·3회 동형 반복 이펙트가 떠요.",
        "도감 수 블록마다 내 chess.com 전적과 승률이 표시돼요.",
        "미니게임 보드에서 기물을 끌어서 옮길 수 있어요.",
      ] },
      { cat: "ui", items: [
        "도감 오프닝 트리의 블록·이름표·연결선이 서로 겹치지 않게 다시 배치됐어요. 같은 부모의 수끼리는 가깝게, 다른 갈래와는 넓게 떨어져 보여요. 이름표는 항상 블록 위에 있어요.",
        "수 이펙트 이름표가 '탁월한 수'로 바뀌고 가운데 정렬됐어요. 칸 기호는 수 체계 아이콘과 같은 모양이에요.",
        "미니게임 준비 화면이 깔끔해졌고, 규칙 설명은 오른쪽 위 ? 버튼을 누르면 말풍선으로 보여요.",
        "분석 탭 수 블록의 일일 퀘스트 표시가 두루마리처럼 펼쳐지고, 가끔씩 흔들려 눌러 보라고 알려줘요.",
        "도감 수 카드가 작아지고, 데스크톱 플레이 탭 버튼도 조금 작아졌어요.",
      ] },
      { cat: "ux", items: [
        "내 퍼즐이 오늘의 퍼즐로 뽑히면 팝업 대신 알림 창에서 몇 번 퍼즐인지와 함께 알려줘요. 도감 잠금 해제 알림은 더 이상 뜨지 않아요.",
        "퍼즐에서 틀리면 빨간 X 이펙트가 뜨고, 좋은 수 이펙트는 끝까지 보여준 뒤 다음 수로 넘어가요.",
        "도감 트리를 끌 때 관성이 붙고, 두 손가락 핀치·Ctrl+휠·더블클릭으로 확대할 수 있어요. 백랭크 러시아워 힌트는 없어졌어요.",
      ] },
      { cat: "perf", items: [
        "도감 오프닝 트리가 열리는 시간이 크게 줄었어요.",
        "수 이펙트와 대국 종료 이펙트가 GPU로 재생돼 더 부드럽고, 20% 빨라졌어요.",
      ] },
      { cat: "fix", items: [
        "기기에서 '애니메이션 줄이기'를 켜 두면 사이트 애니메이션이 멈추던 문제를 고쳤어요 — 다시 생기지 않도록 배포 전에 자동으로 검사해요.",
        "개발자가 추가한 이론 수가 도감에서 이론 수로 표시되지 않던 문제를 고쳤어요 — 이것도 배포 전에 자동으로 검사해요.",
        "도감 수 카드가 화면 밖으로 잘리던 문제, 내 최근 대국의 레이팅 변화가 가끔 틀리게 보이던 문제를 고쳤어요.",
      ] },
    ],
  },
  {
    version: "0.5.5", date: "2026.9.25",
    summary: "플레이 탭이 새 옷을 입었어요 — 일반 대국과 미니게임을 한 화면에 모으고, 좋은 수를 두면 보드 위에 등급 이펙트가 터져요.",
    mascot: {
      intro: { char: "milku", expr: "great", name: "MILKU 코치", align: "left", text: "탁월한 수를 두면 보드가 반짝여요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "나이트 레이스 5라운드, 17초 안에 갈 수 있을까요?" },
    },
    highlight: { kind: "icon", Icon: Sparkles, color: T.brassHi, label: "새 플레이 탭 · 수 등급 이펙트 · 상대 나이트 잡기 · 라운드별 제한시간 · 시각 효과·미니게임 설정" },
    sections: [
      { cat: "feature", items: [
        "탁월한 수·유일한 수·최선의 수를 두면 칸이 등급 색으로 빛나고 '탁월합니다' 같은 이름표가 뜬 뒤 작은 배지로 줄어드는 이펙트가 생겼어요. 분석·학습·퍼즐 탭, 게임 리뷰, 무한 체크메이트 게임에서 볼 수 있어요.",
        "무한 체크메이트 게임은 내가 둔 수를 엔진이 바로 채점해, 메이트 수든 중간 수든 탁월·유일·최선이면 이펙트를 보여줘요.",
        "나이트 레이스에서 상대 나이트를 잡을 수 있어요 — 상대 나이트가 있는 칸으로 뛰어들면 잡히고, 잡힌 쪽은 그 라운드가 끝나요.",
        "설정 탭에 '시각 효과'(수 등급 이펙트, 기본 켜짐)와 '미니게임 설정'(상대 기물이 지키는 칸 표시, 기본 꺼짐)이 생겼어요.",
      ] },
      { cat: "ui", items: [
        "일반 대국과 미니게임을 한 화면에 모았어요. 미니게임 네 개는 가운데 로고를 둘러싼 네 개의 버튼이 되었고, 버튼 속 보드에서는 조준경·나이트 추격·실전 체크메이트·백랭크 러시아워 장면이 계속 움직여요.",
        "일반 대국 버튼 보드에서는 매번 다른 마스터 대국이 재생되고, 누르면 따로 뜨는 창에서 시간을 골라 시작해요.",
        "미니게임 이름이 나이트 레이스·백랭크 러시아워·무한 체크메이트 게임으로 바뀌고, 미니게임 화면이 모두 밝은 크림색이 되었어요.",
        "좌표 인지 게임은 칸을 누르면 조준경이 떴다가 정답은 초록 체크, 오답은 빨간 X로 알려줘요. 무한 체크메이트 게임도 둔 칸에 체크·X가 뜨고, 등급은 아이콘으로 보드 바로 위에 보여줘요.",
        "나이트 레이스에서 나이트가 잡히면 그 칸을 지키던 상대 기물이 날아와 잡는 모습이 보이고, 목표 칸은 반짝이는 금색 별로 바뀌었어요.",
      ] },
      { cat: "ux", items: [
        "나이트 레이스 제한시간이 1라운드 5초부터 라운드마다 늘어나요(최대 17초) — 대신 뒤로 갈수록 기물이 최대 5쌍까지 늘고 목표도 훨씬 멀어져요.",
        "나이트 레이스는 도착하거나 잡힌 장면을 끝까지 보여준 뒤 정산 화면이 뜨고, 혼자 플레이에서는 내 진영 기물이 나오지 않아요.",
        "나이트 레이스·백랭크 러시아워에서 상대 기물이 지키는 칸은 기본으로 숨겨져, 직접 읽어 내는 재미가 생겼어요.",
      ] },
      { cat: "fix", items: [
        "미니게임 랜덤 매칭을 누르면 대기열로 가지 않고 곧장 패배가 뜨던 문제를 고쳤어요 — 같은 문제가 다시 생기지 않도록 배포 전에 자동으로 검사해요.",
        "무한 체크메이트 게임이 시작하자마자 동작하지 않던 문제를 고쳤어요.",
        "무한 체크메이트 버튼 속 장면이 실제로는 메이트가 아닌데 메이트로 넘어가던 문제를 고쳤어요.",
        "백랭크 러시아워 봇 대전에서 결과가 한참 늦게 뜨던 문제를 고쳤어요.",
      ] },
    ],
  },
  {
    ...
    sections: [
      { cat: "feature", items: [
        ...
        "나이트 경주 새 규칙 — 상대 기물 칸에 도달하면 그 기물을 잡아 없애고(그 기물이 막던 칸도 안전해져요), 상대 기물이 지배하는 빨간 칸에 들어가면 내 나이트가 잡혀 그 라운드가 끝나요. 내 색 기물 칸에는 설 수 없어요.",
        "나이트 경주 승패 규칙 — 더 적은 수로 목표에 도착한 쪽이 라운드를 가져가고, 수가 같으면 더 빨리 도착한 쪽이 이겨요.",
        "러시아워 새 규칙 — 주인공 룩이 상대 기물이 지배하는 칸에 들어가면 그 즉시 잡혀 라운드가 끝나요(혼자 풀기는 실패 화면에서 다시 풀기).",
      ] },
      { cat: "ux", items: [
        "나이트 경주 난이도를 크게 올렸어요 — 첫 라운드부터 상대 기물이 나오고, 목표까지 최소 3~6수(라운드가 갈수록 기물 3쌍까지)가 필요해요. 2라운드부터는 가장 빠른 길이 위협 칸으로 막혀 있어 돌아가거나 기물을 잡아 길을 열어야 해요. 이동 수 제한은 최소 수 +1, 제한시간은 25초에서 15초로 줄었어요.",
      ] },
      { cat: "ui", items: [
        ...
        "나이트 경주에서 잡을 수 있는 상대 기물은 금색 고리로 깜빡이고, 잡힌 나이트는 흐려지며 빨간 X가 표시돼요.",
        "나이트 경주·러시아워는 라운드가 끝날 때마다 결과 화면과 같은 디자인의 정산 화면이 떠요 — 나와 상대의 결과·이동 수·걸린 시간을 나란히 비교하고, 이긴 이유와 누적 점수, 다음 라운드까지 남은 시간을 보여줘요.",
      ] },
      { cat: "fix", items: [
  ...
  },
  {
    ...
    mascot: {
      intro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "left", text: "미니게임이 네 개가 됐어요!" },
      outro: { char: "milku", expr: "think", name: "MILKU 코치", align: "right", text: "러시아워 별 138개, 다 모아 볼까요?" },
    },
    highlight: { kind: "icon", Icon: Puzzle, color: T.brassHi, label: "새 미니게임 러시아워 · 새 미니게임 공격 모드 · 미니게임 카운트다운·효과음·진동·애니메이션 · 새 결과 화면" },
    sections: [
      { cat: "feature", items: [
        "새 미니게임 '러시아워' — 내 기물들 사이에 갇힌 룩(왕관 표시)을 빼내 상대 백랭크에서 킹을 체크메이트하는 퍼즐이에요. 다른 기물들은 실제 체스 규칙대로 움직여 길을 비켜 주고, 상대 수비 기물은 방금 움직인 내 기물이 자기 공격 범위에 들어오면 잡으러 오기 때문에 일부러 기물을 내줘 수비를 끌어낼 수도 있어요.",
        "러시아워는 혼자 풀기(쉬움·보통·어려움 46개 레벨, 최단 수 기준 별 3개 평가, 되돌리기·처음부터·위험 칸 보기·힌트), 봇 대전, 실시간 대전, 친구 도전을 모두 지원해요. 대전은 쉬움→보통→어려움 3라운드 2선승이고, 같은 퍼즐을 동시에 풀어 더 적은 수로(같으면 더 빨리) 푼 쪽이 이겨요.",
        "새 미니게임 '공격 모드' — 3분 동안 강제 체크메이트 포지션('공격 기회')이 끝없이 나오고, 더 많이 메이트시킨 쪽이 이겨요. 한 수라도 틀리면 그 기회는 실패하고 정답 칸을 보여준 뒤 바로 다음 기회로 넘어가요.",
        "공격 기회는 메이트가 짧을수록 좋은 등급(S 1수·A 2수·B 3수·C 4수)이고, 퍼즐 레이팅이 낮은 쪽이 좋은 등급을 받을 확률이 더 높아요. 동점이면 낮은 등급(C→B→A→S)의 성공 수부터 비교하고, 그래도 같으면 불리한 확률로 싸운 레이팅 높은 쪽이 이겨요. 봇 대전은 쉬움·보통·어려움 세 단계예요.",
        "네 미니게임 모두 혼자 플레이하기가 생겼어요 — 좌표 인지 게임은 30초 동안 몇 개 맞히는지, 나이트 경주는 5라운드 동안 몇 번 도달하는지(같으면 걸린 시간), 공격 모드는 3분 동안 몇 번 메이트하는지로 기록에 도전하고, 러시아워는 레벨을 풀어요. 최고 기록은 기기에 저장되고, 새 기록을 세우면 '신기록!' 화면이 떠요.",
        "공격 기회 포지션은 실전 마스터 대국에서 뽑아 엔진으로 검증한 295개로 시작하고, 개발자가 리체스 퍼즐에서 가져오거나 FEN을 직접 넣어 계속 늘릴 수 있어요.",
      ] },
      { cat: "ui", items: [
        "미니게임 네 개의 시작 화면을 똑같은 배치로 맞췄어요 — 규칙 안내 아래에 '혼자 플레이하기 · 봇과 플레이하기 · 랜덤 매칭' 세 칸, 그 아래 '친구와 플레이하기' 목록이에요.",
        "미니게임 네 개 모두 라운드 시작 전에 3·2·1 카운트다운이 떠요 — 실시간 대전에서는 두 사람이 같은 시각에 시작하도록 맞춰요.",
        "점수가 오를 때마다 숫자가 튀어 오르는 새 점수판, 라운드가 끝날 때 떠오르는 결과 배너, 남은 시간이 적으면 빨갛게 깜빡이는 시간 막대가 생겼어요.",
        "나이트 경주의 나이트와 러시아워의 기물들이 칸 사이를 미끄러지듯 움직여요.",
        "결과 화면을 새로 만들었어요 — 승리하면 금빛 파티클이 터지고, 라운드별 기록과 게임별 통계(평균·최고 반응속도, 평균 이동 수, 최단 도달 시간, 등급별 성공 수 등)를 보여줘요. 봇 대전은 '다시 하기'로 바로 한 판 더 할 수 있어요.",
      ] },
      { cat: "ux", items: [
        "미니게임에 효과음과 진동이 생겼어요 — 카운트다운, 정답·오답, 라운드 승패, 최종 승리 팡파르까지. 설정의 효과음 켜기/끄기·볼륨을 그대로 따라요.",
        "오답을 누르거나 위험한 칸으로 가려고 하면 보드가 좌우로 흔들려 바로 알 수 있어요.",
        "네 미니게임 모두 봇 대전 난이도를 낮췄어요 — 좌표 인지 게임 봇은 더 늦게 반응하고, 나이트 경주 봇은 더 천천히 움직이며 가끔 헛걸음을 하고, 러시아워·공격 모드 봇은 더 오래 걸리고 더 자주 실패해요.",
      ] },
      { cat: "fix", items: [
  ...
  },
  {
    ...
    summary: "표준 시작 위치에서 두는 모든 대국·퍼즐에서, 킹이 체크를 피해 제자리를 벗어났다가 룩은 안 움직인 채 다시 돌아오면 캐슬링이 다시 가능해지던 규칙 위반을 고쳤어요(FEN 모드에는 원래부터 없던 문제였어요). 퍼즐의 국면(오프닝/미들게임/엔드게임) 판정을 다시 다듬어, 실전 대국에서 온 퍼즐은 몇 수째인지도 반영하고 남은 기물이 얼마 안 되면 항상 엔드게임으로 봐요. 퍼즐 탭 풀이 카드를 닫으면 이제 스크롤해 둔 자리 그대로 돌아오고, FEN 퍼즐 이름을 바꾼 뒤 카드를 닫아도 새 이름이 유지돼요. 개발자의 퍼즐 생성자 회수·양도 기능이 조용히 실패하던 문제, 같은 포지션의 퍼즐이 다른 번호로 중복 생성되던 문제, FEN 퍼즐을 다 풀어도 카드가 초록색이 안 되던 문제까지 함께 고쳤어요. 대국 리뷰 코치 카드에는 '계획' 버튼이 새로 생겨서, 누르면 엔진으로 여러 수에 걸친 기물 재배치 계획을 찾아 보여줘요. 그리고 사용자들이 골라 준 새 기능 여섯 가지 — 일일 퍼즐 스트릭·배지, 적응형 퍼즐 난이도, 오프닝 트리 내 승률 오버레이, 프로필 약점 리포트, 리뷰 공유 이미지 카드, PvP 관전 모드 — 를 한 번에 추가했어요.",
    mascot: {
      intro: { char: "milku", expr: "think", name: "MILKU 코치", align: "left", text: "캐슬링 규칙 허점까지 꼼꼼히 잡았어요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "새 기능 여섯 가지, 하나씩 다 써 보세요!" },
    },
    highlight: { kind: "icon", Icon: Shield, color: T.brassHi, label: "캐슬링 권리 검증 누락 수정 · 퍼즐 국면 판정 개선 · 일일 퍼즐 스트릭·배지 · 적응형 퍼즐 난이도 · 오프닝 트리 내 승률 오버레이 · 프로필 약점 리포트 · 리뷰 공유 이미지 카드 · PvP 관전 모드" },
    sections: [
      { cat: "security", items: [
        "표준 시작 위치에서 두는 모든 대국·퍼즐(실시간 대국, 봇 대국, 퍼즐 풀이, 분석 등)에서, 킹이 체크를 피하는 등으로 제자리를 벗어났다가 다시 원래 칸으로 돌아오면 룩을 한 번도 안 움직였다는 이유만으로 캐슬링이 다시 가능해지던(실제 체스 규칙 위반) 문제를 고쳤어요 — 실시간 대국에서 악용될 수 있는 허점이었어요.",
      ] },
      { cat: "fix", items: [
      ...
      { cat: "feature", items: [
        ...
        "일일 퍼즐을 매일 이어 풀면 연속 기록(스트릭)과 배지가 쌓여요 — 퍼즐 탭에서 불꽃 아이콘과 함께 확인할 수 있고, 최고 기록도 따로 저장돼요.",
        "퍼즐 추천 난이도가 최근 컨디션(정답률 흐름)과 풀이 경험치에 맞춰 스스로 조금씩 조정돼요 — 최근 잘 풀고 있으면 살짝 더 어려운 쪽으로, 어려워하고 있으면 살짝 쉬운 쪽으로 목표 레이팅을 밀어줘요.",
        "도감 오프닝 모식도의 각 수 블록에 chess.com 전적 기준 내 실제 승률이 색으로 표시돼요(세계적인 채택률·전적은 그대로 함께 보여요).",
        "프로필의 chess.com 통계 화면에 '약점 리포트'가 추가됐어요 — 그동안 리뷰한 대국들을 모아 블런더가 유독 잦은 오프닝을 알려줘요.",
        "대국 리뷰 요약을 SNS에 바로 올릴 수 있는 이미지 카드로 내보낼 수 있어요 — 체스보드 느낌의 디자인에 OpenChess 로고·기물 아이콘, 실제 상대의 프로필 사진·레이팅, 한 수씩 색으로 보여주는 대국 흐름, '분석' 탭과 똑같은 수 등급별 목록까지 함께 담겨요.",
        "친구가 지금 하고 있는 실시간 대국을 참가하지 않고 구경만 할 수 있는 '관전' 기능이 생겼어요 — 친구 목록에서 대국 중인 친구에게 뜨는 관전 버튼을 누르면 돼요.",
      ] },
    ],
  },
  {
    version: "0.5.1", date: "2026.9.13",
    summary: "Stockfish 18 엔진을 고르면 항상 '연결 실패'로 멈추던 문제(신경망 파일을 외부 저장소에서 받아오던 방식이 계속 말썽이어서, 다른 두 엔진처럼 사이트와 같은 곳에서 바로 받아오도록 되돌렸어요)와, 퍼즐 이름을 바꿔도 다른 화면(특히 '내가 만든 퍼즐' 목록, 오늘의 퍼즐)에서는 예전 이름이 계속 보이던 문제를 고쳤어요. 그리고 실시간 대국 상대가 시간이 다 됐는데도 결과를 보고하지 않고 화면을 나가버리면(인터넷이 끊기거나 탭을 닫는 등) 그 대국이 서버에 끝나지 않은 것처럼 계속 남아, 이긴 사람이 다음 매칭을 시도할 때마다 계속 그 끝난 대국으로 되돌아가던 문제를 근본적으로 고쳤어요 — 이제 서버가 양쪽의 남은 시간을 직접 기억해 뒀다가, 누가 접속해 있든(이긴 쪽이든 진 쪽이든) 시간이 다 됐다는 걸 스스로 확인하고 즉시 결과를 확정해요. 분석 탭 'FEN 모드'에도 이제 엔진이 계산한 다음 수·현재 수 정보가 정확한 등급으로 뜨고(다음 수 블록과 현재 수 블록의 등급이 서로 다르게 보이던 문제까지 마저 고쳤어요), 둘 수 있는 수가 1~2개뿐인 국면에서 엔진 상위 줄이 불필요하게 가운데 떠 보이던 것도 정리했어요. 이미지 스캔(체스판·PGN·FEN 사진 인식) 속도를 한 번 더 높이면서 진행률(%)도 실시간으로 보여줘요. 보드 편집기에서는 킹·룩이 시작 칸을 벗어나면 캐슬링 권리가 항상 자동으로 꺼지고, 폰 승격 선택 창도 승격하는 진영 색에 맞는 기물 아이콘으로 더 크고 정확한 위치에 떠요. 좌표 인지 게임·나이트 경주 두 미니게임도 화면 전체를 쓰는 별도 화면에서 플레이하도록 다시 디자인했어요 — 모바일에서는 내 보드가 아래쪽, 상대(또는 봇) 보드가 위쪽에 오고, 화면 크기가 얼마든 두 보드가 스크롤 없이 한 번에 다 보이도록 자동으로 맞춰져요.",
    mascot: {
      intro: { char: "milku", expr: "think", name: "MILKU 코치", align: "left", text: "이제 Stockfish 18도 제대로 켜져요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "미니게임도 이제 전체화면으로 즐겨요!" },
    },
    highlight: { kind: "icon", Icon: Shield, color: T.brassHi, label: "Stockfish 18 연결 실패 · 퍼즐 이름 되돌림 · 실시간 대국 시간 초과 좀비 대국 근본 수정 · FEN 모드 다음 수·수 체계 아이콘 정확도 개선 · 이미지 스캔 속도·진행률 개선 · 캐슬링 권리 자동 해제 · 승격 선택 창 색상·크기·위치 개선 · 미니게임 전체화면 레이아웃 재설계" },
    sections: [
      { cat: "fix", items: [
        "Stockfish 18(정식) 분석 엔진을 고르면 항상 '연결 실패'로 멈추던 문제를 고쳤어요 — 신경망 파일(108MB)을 외부 저장소에서 받아오던 방식이 계속 말썽이어서(다른 저장소로 몇 번을 옮겨봐도 계속 안 됐어요), 다른 두 엔진처럼 사이트와 같은 곳에서 바로 받아오도록 되돌렸어요. 혹시라도 부팅이 오래 걸리는 경우를 대비해 설정 탭에 안내 문구도 추가했어요.",
        "퍼즐 이름을 바꿔도 다른 화면(특히 '내가 만든 퍼즐' 목록, 오늘의 퍼즐)에서는 바꾸기 전 이름이 계속 보이던 문제를 고쳤어요.",
        "실시간 대국에서 상대의 시간이 다 됐는데도 상대 화면이 결과를 보고하지 못하고 사라지면(인터넷 끊김, 탭 닫기 등), 예전엔 그 대국이 끝나지 않은 것으로 서버에 계속 남아 다음 매칭 시도가 자꾸 그 끝난 대국으로 되돌아갔어요 — 이제 서버가 양쪽 진영의 남은 시간을 직접 기억해 두고, 이긴 쪽 화면이 대신 확인 요청을 보내도 안전하게(클라이언트의 주장을 믿지 않고 서버가 독립적으로 계산해서) 결과를 확정해요.",
        "분석 탭 'FEN 모드'에서 다음 수 칸이 항상 비어 있던 문제, 그리고 그 자리에 전혀 무관한 표준 오프닝 추천 문구가 잘못 섞여 보이던 문제를 함께 고쳤어요 — 이제 엔진이 계산한 실제 후보 수가 뜨고, 수를 두면 현재 수 블록도 정확히 표시돼요.",
        ...
        "FEN 모드의 '다음 수' 블록에서 수 체계 아이콘(최선·탁월·좋은 수 등)이 부정확하게 뜨던 문제를 고쳤어요 — 예전엔 1순위 후보만 무조건 '최선의 수', 나머지는 전부 '좋은 수'로만 단순 표시했는데, 실제 손실값과 희생 여부까지 반영해 다른 화면(게임 리뷰 등)과 똑같은 규칙으로 정확하게 등급이 매겨져요.",
        "폰이 승격할 때 뜨는 기물 선택 창이 항상 검은 기물 아이콘만 보여주던 문제를 고쳤어요 — 이제 백이 승격하면 흰 기물, 흑이 승격하면 검은 기물 아이콘이 떠요.",
        "FEN 모드에서 '다음 수' 블록에 뜬 등급과 그 수를 실제로 둔 뒤 현재 수 블록에 뜨는 등급이 서로 다르게(예: 다음 수 블록엔 '최선의 수'인데 현재 수 블록엔 '좋은 수') 표시되던 문제를 고쳤어요 — 두 블록이 이제 항상 같은 등급을 보여줘요.",
      ] },
      { cat: "perf", items: [
        "이미지 스캔(체스판·PGN·FEN 사진 인식) 속도를 더 높였어요 — 서버로 보내기 전에 사진을 더 작은 크기로 줄이고, 인식이 애매해 다시 시도할 때도 여러 번을 순서대로 기다리는 대신 한꺼번에 보내도록 바꿨어요.",
        "이미지 스캔 속도를 한 번 더 높였어요 — 서버로 보내기 전 사진 압축률을 더 올렸어요.",
      ] },
      { cat: "feature", items: [
        "이미지 스캔이 진행되는 동안 지금 몇 %까지 됐는지 실시간으로 보여줘요 — 업로드가 끝난 뒤 인식 엔진의 응답을 기다리는 구간은 정확한 값을 알 방법이 없어 서서히 채워지는 것처럼 흉내만 내다가, 실제 응답이 오면 곧장 100%로 넘어가요.",
        "보드 편집기에서 킹이나 룩을 시작 칸(e1/h1/a1, e8/h8/a8)에서 벗어나게 두면 그 캐슬링 권리가 항상 자동으로 꺼져요. 반대로 다시 제자리로 돌아온다고 권리가 되살아나지는 않아요 — 실제 체스 규칙과 같은 태도예요.",
      ] },
      { cat: "ui", items: [
        "분석 탭 'FEN 모드'에서 화면 위쪽에 따로 떠 있던 안내 박스를 없앴어요 — 그 안내가 담고 있던 종료 버튼을 바로 아래 FEN 코드 줄로 옮겼고, 복사 버튼 위치도 FEN·PGN 두 줄에서 항상 같은 자리에 오도록 정리했어요.",
        "폰 승격 선택 창의 기물 버튼을 더 크게 키우고, 체스보드(잡힌 기물 줄·엔진 평가 줄 등을 뺀 실제 8x8 판) 정중앙에 정확히 뜨도록 고쳤어요 — 예전엔 그 판을 감싼 더 넓은 영역의 중앙에 떠서 실제 보드 중앙과 어긋나 보였어요.",
        "둘 수 있는 수가 1~2개뿐인 국면에서 보드 위 엔진 상위 줄이 실제 수보다 더 넓은 자리(항상 3줄 몫)를 차지해 그 아래 빈 여백이 남던 문제를 고쳤어요 — 이제 실제로 있는 수만큼만 자리를 차지해서, 있는 줄들이 위에서부터 그 자리에 그대로 표시돼요.",
  ...
  },
  {
    ...
    summary: "'상점' 탭이 '플레이' 탭으로 개편됐어요 — 분석 탭의 PLAY 버튼과 똑같은 화면으로 곧장 들어가고, 그 화면을 아래로 내리면 기존 상점(스킨) 화면이 그대로 이어져요. 플레이 페이지 맨 위에는 '일반/스페셜' 토글이 생겼고, 스페셜에서는 실시간 대전 미니게임 두 가지를 한 줄씩 만나볼 수 있어요 — 보드 아래 뜨는 좌표를 읽고 상대보다 먼저 그 칸을 클릭하는 '좌표 인지 게임', 나이트로 목표 칸까지 상대보다 먼저 도달하는 5전 3선승 '나이트 경주'예요. 두 미니게임 모두 체스처럼 봇과 바로 대전하거나 친구에게 도전장을 보낼 수 있어요. 플레이 탭도 이제 다른 탭처럼 상단 헤더·하단 탭바가 함께 보여요(예전엔 화면 전체를 덮는 별도 화면이었어요) — 대국 중 다른 탭을 둘러봐도 진행 중이던 대국은 끊기지 않아요. 하단 탭 순서도 분석·플레이·퍼즐·학습·도감·설정으로 정리했고, 도감 탭 오프닝 모식도의 흰 영역을 데스크톱에서 조금 더 넓혔어요. 분석 탭 수 블록의 리체스 채택률(%)이 반올림 때문에 부정확하게 보이던 것도 고쳐, 이제 소수점 둘째 자리까지 실제 값에 더 가깝게 보여요. 모바일에서 분석 탭 엔진 라인(상위 3줄)이 안 뜨던 문제도 고쳤어요. 그리고 퍼즐 삭제, FEN 퍼즐 이름 변경이 안 되던 버그를 근본 원인부터 고쳤어요 — 알고 보니 서버가 '완료했다'는 응답을 빈 몸으로 보내는 요청 전부를, 앱이 그 빈 응답을 읽으려다 실패해서 무조건 '실패'로 잘못 판단하고 있었어요. 같은 이유로 조용히 안 되고 있던 계정 탈퇴, 채팅 메시지 수정·대화 지우기, 실시간 대국 대기열/초대 취소 같은 기능들과, 내가 만든 퍼즐이 오늘의 퍼즐로 뽑혔는데도 알림이 안 오던 문제도 이번에 함께 고쳐졌어요. 두 미니게임 모두 이제 상대(또는 봇)의 보드를 내 보드 아래에 따로 보여줘서, 좌표 인지 게임은 상대의 클릭이, 나이트 경주는 상대 나이트의 실시간 이동이 눈에 보여요. 점수 아래에는 라운드별 승패를 점으로 표시하는 진행 상황 바도 생겼어요.",
    mascot: {
      intro: { char: "milku", expr: "great", name: "MILKU 코치", align: "left", text: "좌표 인지 게임·나이트 경주로 친구와 실시간 대전해요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "채택률 표시도 더 정확해졌어요!" },
    },
    highlight: { kind: "icon", Icon: Target, color: T.brassHi, label: "상점 탭 → 플레이 탭 개편 + 실시간 미니게임 '좌표 인지 게임'·'나이트 경주' + 채택률·모바일 엔진 라인 등 버그 수정" },
    sections: [
      { cat: "feature", items: [
        "'상점' 탭이 '플레이' 탭으로 바뀌었어요 — 분석 탭의 PLAY 버튼을 누른 것과 완전히 같은 대국 설정 화면으로 곧장 들어가요.",
        "그 대국 설정 화면을 아래로 내리면 기존 상점(체스보드·기물 스킨) 화면이 그대로 이어져 있어요.",
        "플레이 페이지 맨 위에 '일반/스페셜' 토글이 새로 생겼어요. 스페셜의 첫 실시간 대전 미니게임 '좌표 인지 게임'에서는 보드 아래 뜨는 좌표를 읽고 상대보다 먼저 그 칸을 클릭해야 해요 — 15라운드를 먼저 더 많이 맞히는 쪽이 승리해요.",
        "스페셜에 두 번째 실시간 대전 미니게임 '나이트 경주'가 추가됐어요 — 나이트로 목표 칸까지 상대보다 먼저 도달하는 5전 3선승 대결이에요. 라운드가 진행될수록 방해 칸이 늘어나 점점 어려워져요.",
        "좌표 인지 게임·나이트 경주 두 미니게임에도 체스처럼 '봇과 플레이하기'·'친구와 플레이하기'가 생겼어요 — 봇은 서버 없이 그 자리에서 바로 대전할 수 있고, 친구에게는 도전장을 보내 응답을 기다릴 수 있어요.",
      ] },
      { cat: "ui", items: [
        "플레이 탭도 다른 탭처럼 상단 헤더·하단 탭바가 함께 보이도록 바꿨어요 — 예전엔 화면 전체를 덮는 별도 화면이었어요. 대국 중 다른 탭을 둘러봐도 진행 중이던 대국은 끊기지 않고 그대로 이어져요.",
        "스페셜 미니게임 목록이 카드 그리드에서 한 줄에 게임 하나씩 보여주는 목록으로 바뀌었어요 — 좌표 인지 게임은 청록색 과녁 아이콘, 나이트 경주는 파란색 경로 아이콘으로 각 게임의 특징을 한눈에 구분할 수 있어요. 테스트용 예시 게임은 정리했어요.",
        ...
        "좌표 인지 게임에서 목표 칸이 보드 위에서 빛나는 대신, 그 좌표를 보드 아래에 텍스트로 표시하도록 바꿨어요 — 반짝임을 따라 누르는 대신 좌표를 직접 읽고 찾아 눌러야 해요.",
        ...
        "하단 탭 순서를 분석·플레이·퍼즐·학습·도감·설정으로 바꿨어요.",
        "도감 탭에서 오프닝 모식도가 표시되는 흰 영역이 데스크톱에서 조금 더 넓어졌어요.",
      ] },
      { cat: "fix", items: [
        "분석 탭 수 블록의 리체스 채택률(%)이 반올림 때문에 실제보다 부정확하게 보이던 문제를 고쳤어요 — 이제 소수점 둘째 자리까지 실제 값에 더 가깝게 표시돼요.",
        "모바일에서 분석 탭의 엔진 라인(상위 3줄)이 뜨지 않던 문제를 고쳤어요.",
        "퍼즐 삭제 버튼을 눌러도 지워지지 않던 문제를 고쳤어요.",
        "FEN으로 만든 퍼즐의 이름을 바꿔도 저장되지 않던 문제를 고쳤어요.",
        "같은 원인으로 조용히 실패하고 있던 계정 탈퇴, 채팅 메시지 수정·대화 지우기, 실시간 대국 대기열·초대 취소 등도 함께 정상화됐어요.",
  ...
  },
  {
    ...
    summary: "이 about 페이지 버전 기록에서 빠져 있던 v0.4.8 항목을 채워 넣었어요. 분석 탭에서 자주 두어지는 다음 수의 리체스 통계를 미리 백그라운드에서 당겨와 둬서 통계 표시가 훨씬 빨라졌고, 현재 수 블록·다음 수 블록의 회수(a/b) 표기가 자릿수 때문에 잘려 보이던 문제도 게이지 바를 더 줄여서 고쳤어요(불러오는 동안엔 3-dot bounce 인디케이터가 대신 떠요). 분석 탭 메인 보드가 모바일 화면을 카드 테두리까지 꽉 채우도록 훨씬 커졌고, 그 아래 버튼도 함께 커졌어요. 평가치 막대·엔진 라인이 보이거나 안 보이거나, 엔진 라인이 새로 생기거나 순위가 바뀌거나 — 어떤 경우에도 보드 좌표가 흔들리지 않도록 두 자리 모두 픽셀 단위로 고정했어요. 일일 퍼즐 팝업·퍼즐 풀이 카드에서 기보를 눌러 집중 분석으로 이동했다가 나가면 이제 들어왔던 화면으로 정확히 돌아가고, 퍼즐 카드의 레이팅 배지·테마 장식도 다듬었어요. FEN으로 만든 퍼즐도 이름을 직접 짓고 그 포지션을 학습 탭에서 바로 볼 수 있게 됐고, 퍼즐 레이팅 계산식도 더 합리적으로 조정했어요. 채팅 블라인드 대국은 연속 두기를 막고 무승부도 상호 동의가 필요하도록 다듬었어요. 그리고 가장 큰 변화 — 일일 퍼즐을 개발자가 수동으로 배정하던 방식을 버리고, 커뮤니티가 만든 인기 퍼즐 중 하나를 매일 밤 자동으로 골라 확정하는 방식으로 전면 개편했어요. 선정된 제작자에게는 알림과 함께 코인 보상을 드려요.",
    mascot: {
      intro: { char: "milku", expr: "think", name: "MILKU 코치", align: "left", text: "분석 탭 보드가 모바일 화면을 훨씬 넓게 채워요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "내가 만든 퍼즐이 일일 퍼즐로 뽑히면 코인을 받을 수 있어요!" },
    },
    highlight: { kind: "icon", Icon: Zap, color: T.brassHi, label: "일일 퍼즐 커뮤니티 자동 선정 + FEN 퍼즐 이름·표시 + 분석 탭 보드 확대·흔들림 고정" },
    sections: [
      { cat: "feature", items: [
        "FEN으로 만든 퍼즐도 생성자·개발자가 이름을 직접 지을 수 있어요(수 설명과 같은 금칙어 검사가 적용돼요) — 권한이 있으면 이름 옆에 펜 아이콘이 떠요.",
        "FEN 퍼즐의 풀이 카드에는 기보 대신 FEN 코드가 표시되고, 누르면 학습 탭으로 이동해 그 포지션이 보드에 곧바로 로드돼요.",
        "일일 퍼즐을 정하는 방식을 완전히 바꿨어요 — 개발자가 오프닝 테마를 미리 등록해 두던 방식 대신, 커뮤니티가 만들어 공개한 퍼즐 중 인기 점수가 가장 높은(아직 뽑힌 적 없는) 것을 매일 밤 자정 직전에 자동으로 골라 다음 날 일일 퍼즐로 확정해요.",
        "내가 만든 퍼즐이 일일 퍼즐로 선정되면 알림이 오고 OC 나이트 코인을 받을 수 있어요 — 선정된 순간 접속해 있지 않았어도, 다음에 접속하면 '받기' 버튼이 있는 팝업이 떠서 받기 전까지 계속 알려줘요.",
      ] },
      { cat: "fix", items: [
        "about 페이지 버전 기록에서 빠져 있던 v0.4.8 항목을 채워 넣었어요.",
        "분석 탭 현재 수 블록·다음 수 블록의 회수(a/b) 표기가 숫자 자릿수가 많아지면 말줄임(…)으로 잘려 보이고 채택률(%) 오른쪽 여백이 부족하던 문제를 고쳤어요 — 왼쪽 채택률 게이지 바 길이를 더 줄이고, 그만큼 확보한 자리에 회수 전체 값과 % 여백을 다 보여줘요.",
        "분석 탭에서 평가치 막대나 엔진 라인이 잠깐 사라질 때 그 아래 보드가 위로 들썩이던 문제를 고쳤어요 — 두 자리 모두 항상 같은 높이로 고정돼, 보이든 안 보이든 보드 좌표가 절대 움직이지 않아요.",
        "분석 탭에서 엔진 라인이 새로 생기거나 순위가 바뀔 때 보드가 흔들리던 문제를 근본적으로 고쳤어요 — 계산 중 점 애니메이션·실제 수순·빈 칸, 세 가지 줄 상태 모두 픽셀 단위로 완전히 같은 높이를 갖도록 고정했어요.",
        "분석 탭에서 수를 둘 때, 특히 하단 수 블록에 키워드가 많이 뜨는 포지션일수록 보드 크기가 미세하게 흔들리던 문제를 고쳤어요 — 다음 수 목록의 텍스트가 보드가 속한 grid 트랙 폭에 영향을 주지 않도록 막았어요.",
        "분석 탭 평가치 막대의 '탐색 상태' 도움말 말풍선이 안 뜨던 문제를 고쳤어요.",
        "일일 퍼즐 팝업·퍼즐 풀이 카드의 기보에서 수를 눌러 집중 분석으로 이동했다가 나가면, 예전엔 항상 학습 탭 첫 화면으로 나가졌는데 이제 들어왔던 화면(그 팝업 또는 풀던 퍼즐)으로 정확히 돌아가요.",
        "퍼즐 풀이 카드·퍼즐 카드의 수 재생 애니메이션에서 기물을 잡는 수가 간헐적으로 '잡는 순간'을 표시하지 않고 그냥 미끄러져 가는 것처럼 보이던 문제를 고쳤어요.",
        "FEN 퍼즐에서 컴퓨터가 응수할 때 보드에 순간적으로 전체 기물이 표시되던 문제를 고쳤어요.",
        "FEN 퍼즐의 풀이 카드 FEN 코드를 처음 눌렀을 때 학습 탭으로 이동하지 않고 그대로 남아 있던 문제를 고쳤어요(예전엔 두 번째 클릭부터 이동했어요).",
        "짧고 단순한 퍼즐의 레이팅이 기물 수가 많다는 이유만으로 지나치게 높게 매겨지던 문제를 완화했어요 — 기물 수 비중을 줄이고 라인 길이·응수 갈래 수 비중을 늘렸어요.",
        "채팅 블라인드 대국의 '/draw'가 상대 의사와 무관하게 곧바로 무승부로 처리되던 문제를 고쳤어요 — 이제 상대가 실제로 동의해야 무승부가 성립해요.",
      ] },
      { cat: "ui", items: [
        "분석 탭 메인 체스보드가 모바일에서 항상 360px로 묶여 있어 화면이 넓은 기기일수록 좌우 여백만 남던 문제를 고쳤어요 — 이제 카드 테두리까지 꽉 채우는 크기로 커져요(데스크톱의 좌우 2단 레이아웃에서는 기존과 동일하게 360px로 유지돼요).",
        "보드가 커진 만큼 그 아래 뒤집기·초기화·PLAY·뒤로·앞으로 버튼 크기도 함께 커졌어요.",
        "퍼즐 카드의 레이팅 표기를 퍼즐 풀이 카드와 완전히 같은 배지 디자인으로 통일했어요.",
        "퍼즐 테마(기물 희생하기·우위 점하기·실수 응징하기)마다 카드 배경에 다르게 떠 있던 장식용 수 기호(!!·?!·?·??)를 없앴어요.",
        "퍼즐 풀이 카드의 제작자 아이디를 더 크게 표시하고, 눌러서 그 사람의 프로필로 바로 이동할 수 있게 했어요(나오면 풀던 퍼즐로 정확히 돌아와요). PGN/FEN·포지션·레이팅 배지와 공유 버튼은 오른쪽으로 정렬했어요.",
        "퍼즐 풀이 카드에서 코치 말풍선을 화면 안에서 보였다 숨겼다 하던 버튼을 없애고, 기본값도 숨김으로 바꿨어요 — 이제 설정 탭의 '코치 말풍선' 토글로만 켜고 끌 수 있어요.",
        "채팅 말풍선이 길어져도 화면 중앙을 넘어가지 않고 자동으로 줄바꿈되도록 최대 폭을 제한했어요.",
        "/user 페이지 친구 요청 상태 칸에 남아 있던 안내 문구('친구가 되었습니다' 등)를 모두 지우고 아이콘만 남겼어요.",
        "설정 탭 개발자 패널의 '2주 오프닝 테마' 등록 화면을, 최근 자동 선정 내역 확인 + 테스트용 즉시 실행 버튼으로 바꿨어요.",
      ] },
      { cat: "perf", items: [
        "분석 탭에서 실제로 가장 많이 두어진(채택률 상위) 후보 수들의 리체스 통계를 클릭하기 전에 미리 백그라운드에서 당겨와 둬요 — 그 수를 실제로 눌렀을 때 다시 서버에 묻지 않고 곧장 표시돼 체감 로딩 시간이 크게 줄어요.",
        "일일 퍼즐이 이미 만들어진 커뮤니티 퍼즐 데이터를 그대로 재사용하면서, 매번 로컬 엔진으로 다시 계산할 필요가 없어져 더 빠르게 떠요.",
        "Stockfish 18 엔진 파일을 외부 저장소로 옮겨 배포 결과물 용량을 크게 줄였어요 — 사이트를 쓰는 데는 변화가 없어요.",
      ] },
      { cat: "ux", items: [
        "분석 탭 현재 수 블록·다음 수 블록에서 리체스 통계가 아직 도착하기 전에는 회수·채택률 자리가 빈 값(—)이 아니라 3-dot bounce 인디케이터로 로딩 중임을 보여줘요.",
        "채팅 블라인드 대국에서 내가 방금 수를 뒀다면, 상대가 응수하기 전까지는 내 채팅이 SAN 수로 인식되지 않아요(같은 사람이 연달아 두는 것 방지).",
      ] },
    ],
  },
  {
    version: "0.4.8", date: "2026.9.6",
    summary: "채팅으로 \"1.e4\"처럼 수순을 보내면 시작되는 '블라인드 대국'이 생겼어요 — 체크메이트·무승부는 자동으로 판정되고, /resign·/draw·/eval 명령어도 써요. 학습 탭 FEN 모드에서도 막혀 있던 대국 리뷰(/review)가 다시 열리고, 채팅 리뷰 공유 카드의 chess.com 대국 표시가 /user 통계 행과 완전히 같아졌어요. 채팅 입력창에 '/'만 입력해도 명령어 안내가 다시 뜨고, 퍼즐 컨트롤 센터의 손상 검사가 실제 수순을 끝까지 재생해보는 방식으로 더 엄격해졌어요.",
    mascot: {
      intro: { char: "milku", expr: "think", name: "MILKU 코치", align: "left", text: "채팅에 \"1.e4\"처럼 수순을 보내기만 하면 블라인드 대국이 시작돼요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "FEN으로 만든 포지션도 이제 대국 리뷰로 꼼꼼히 채점받을 수 있어요!" },
    },
    highlight: { kind: "icon", Icon: Sparkles, color: T.brassHi, label: "채팅으로 블라인드 대국 + FEN 모드 리뷰 복원 + 리뷰 카드 통일" },
    sections: [
      { cat: "feature", items: [
      ...
      { cat: "fix", items: [
        "학습 탭 FEN 모드에서도 대국 리뷰(/review)를 열 수 있도록 복원했어요 — 채점 로직 자체는 이미 완성돼 있었는데 진입 경로만 막혀 있었어요.",
        "채팅 입력창에 '/'만 입력해도 다시 위에 사용 가능한 명령어 안내가 떠요.",
        "퍼즐 컨트롤 센터의 손상 검사를 강화했어요 — 트리·라인 구조는 있어도 그 안의 수순이 실제로는 둘 수 없는(불법인) 퍼즐을, 처음부터 끝까지 실제로 재생해보고서야 정상/손상을 가려요.",
      ] },
      { cat: "ui", items: [
  ...
  },
  {
    ...
    summary: "분석 탭 체스보드가 수를 둘 때마다 미세하게 흔들리던 문제를 고쳤어요. 실시간 대국에 무승부 제안·재대결 신청·나가기 확인이 새로 생겼어요. 상단 헤더 버튼 테두리·정렬 문제, 수 키워드 말풍선 위치, 친구 요청 버튼 표시를 고쳤고, 채팅 이모티콘 박스가 전체 폭으로 커지며 MILKU&KOKOA를 한 판에서 골라요. OC 나이트 코인 아이콘이 바뀌었고, 수 설명에 좋아요·싫어요와 3가지 정렬이 생겼어요. 개발자 권한 퍼즐 양도 버그를 고쳤고, 개발자 도구에 손상된 퍼즐을 찾아 지우는 퍼즐 컨트롤 센터가 생겼어요.",
    mascot: {
      intro: { char: "milku", expr: "think", name: "MILKU 코치", align: "left", text: "이제 실시간 대국에서 무승부를 제안하거나 재대결을 신청할 수 있어요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "수 설명에 좋아요·싫어요를 눌러서 제일 도움이 되는 설명을 위로 올려주세요!" },
    },
    highlight: { kind: "icon", Icon: Wrench, color: T.brassHi, label: "실시간 대국 무승부·재대결 + 수 설명 좋아요/싫어요 + 여러 버그 수정" },
    sections: [
      { cat: "fix", items: [
      ...
      { cat: "feature", items: [
        "/play에서 봇이 아닌 실시간 상대와 대국할 때는 '다시 설정' 버튼 대신 무승부 제안 버튼이 떠요 — 제안받은 쪽에는 화면 아래에 수락/거절 알림이 표시돼요.",
  ...
  },
  {
    ...
    summary: "/play 앙파상 버그를 고치고 항상 표준 시작 위치에서만 시작되도록 했어요. 친구 초대 링크가 더 편해졌고, '푼 퍼즐' 카드에 좋아요·리포스트·공유 버튼이 생겼어요. 채팅이 크게 개선됐어요 — 프로필 사진·아이디가 커지고 클릭으로 프로필 이동이 되며, 리뷰 공유 카드·명령어·대국 신청 카드가 다시 디자인됐어요. 학습 탭 수 블록·키워드 말풍선·도감 카드·티어 여정 화면도 다듬어졌고, 퍼즐 탭에 '인기순' 정렬이 새로 생겼어요. 헤더 버튼 정렬·URL 경로·티어 여정 화면·수 블록 채택률 애니메이션 등 여러 세부 버그도 마저 고쳤어요.",
    mascot: {
    ...
    sections: [
      { cat: "fix", items: [
        ...
        "상단 헤더 세 버튼의 테두리를 그리는 방식을 바꿔, 프로필 버튼이 나머지 둘보다 살짝 위로 걸쳐 보이던 정렬 문제를 완전히 고쳤어요.",
        "/user 페이지에서 티어 이미지를 클릭했을 때 티어 여정 화면이 뒤에 깔려 보이던 버그를 고쳤어요.",
        "분석 탭과 학습 탭의 주소(/analysis, /learn)가 서로 뒤바뀌어 있던 것을 바로잡았어요.",
        "학습 탭에 처음 들어갈 때 화면 왼쪽 위가 순간적으로 확대되는 것처럼 보이던 버그와, 그때 상단 검색·친구·채팅 버튼의 윤곽선이 흐릿해 보이던 버그를 함께 고쳤어요.",
        "같은 포지션의 리체스 통계를 서버(프록시)에서도 캐싱해, 학습 탭 채택률 애니메이션이 시작되기까지 걸리던 5~10초 지연을 크게 줄였어요.",
      ] },
      { cat: "security", items: [
        "/play는 이제 항상 똑같은 표준 시작 위치에서만 시작돼요 — 보드를 직접 편집했거나 FEN/PGN을 불러와 다른 포지션·수순이 돼 있으면 PLAY 버튼이 비활성화돼요.",
      ] },
      { cat: "feature", items: [
        "친구 초대 링크 박스가 개선됐어요 — 링크 전체가 항상 보이고, 복사 버튼 아래 공유 버튼(브라우저 공유 시트)이 추가됐고, 친구 모달 '추가' 탭에서도 초대 링크를 바로 보낼 수 있어요. 로그아웃 상태로 초대 링크를 열면 설정 탭으로 이동해 로그인이 필요하다고 알려줘요. 검색 박스가 초대 링크 박스보다 위에 표시돼요.",
      ...
      { cat: "ui", items: [
        ...
        "학습 탭 수 블록의 채택률 게이지 바 폭을 살짝 줄이고, 그 여백만큼 회수(a/b)와 채택률(n%) 텍스트 사이 간격을 넓혔어요. 최상단 '현재 수' 블록에도 일반 수 블록과 같은 통계 애니메이션·소수점 둘째 자리 채택률을 적용했어요. 채택률(%) 텍스트는 항상 블록 오른쪽 끝에 정렬되고, 이제 세어 올라가는 애니메이션도 적용돼요.",
        "/user 페이지 전용 전환 토글 크기를 살짝 더 키웠어요(이전 두 크기의 중간).",
        ...
        "/user 페이지 전용 전환 토글이 절반 크기로 줄었고, 데스크톱에서는 티어·퍼즐 레이팅이 왼쪽 유저 정보 아래 항상 표시돼요. 좌우 열 여백이 3배로 넓어지고, 친구 요청 버튼이 프로필 사진과 같은 높이로 오른쪽 정렬돼요.",
        "/user·프로필 카드에서 소개나 실명이 없어도 이름 위치가 흔들리지 않아요.",
        "채팅에서 마우스만 스쳐도 '@아이디' 멘션 프로필로 이동해 버리던 버그를 고쳤어요. 리뷰 공유 카드의 금색 버튼은 채팅창을 닫고 학습 탭에 기보를 바로 입력하고, 초록 버튼은 리뷰 페이지로 이동해요.",
        "설정 탭 개발진 블록에서 개발자 아이디를 누르면 그 아이디의 프로필로 이동해요. 이제 저장된 소문자 계정 아이디 대신 실제 표시용 아이디로 나와요.",
        "상단 헤더 검색·친구·채팅 버튼 묶음의 테두리가 안 보이던 문제와, 그랜드마스터 계정에서 프로필 버튼 높이가 유독 달라 보이던 문제를 고쳤어요.",
        "/user 페이지 순위 배지를 없앴어요. chess.com 통계의 닉네임·소개·최근 접속 텍스트 y좌표를 OpenChess 통계와 완전히 통일했고, 국적 표시 박스를 없앴고, 아이디가 덜 잘리도록 더 길게 표시돼요. 티어·퍼즐 레이팅은 이제 OpenChess 통계에서만 보이고 chess.com 통계에서는 같은 자리에 래피드/블리츠/불릿 레이팅이 대신 보여요.",
        "/user 페이지에서 티어 아이콘을 눌러도 티어 여정 화면이 안 보이던 문제(화면 자체는 열렸지만 항상 그 아래에 그려지고 있었어요)를 고쳤어요.",
      ] },
    ],
  },
  {
