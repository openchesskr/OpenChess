...
import {
  ...
  Send, Compass, Rocket, Star, Gem, Pin, Share2, Pencil, Monitor,
} from "lucide-react";
...
const VERSION_HISTORY = [
  {
    ...
    summary: "게임 리뷰 화면이 데스크톱에서는 화면 전체를 넓게 쓰는 전용 레이아웃으로 바뀌었어요 — 체스보드가 훨씬 커지고, 진입 애니메이션의 체스보드·정확도 그래프도 큰 화면에 맞게 커졌어요. 리뷰에 들어갈 때 보여주는 평가치·정확도 그래프 세 개가 서로 비슷한 크기로 보이도록 균형을 맞췄고, 데스크톱 리뷰 화면에서는 평가치 그래프가 조금 더 작아지고 코치 설명이 그 위쪽으로 옮겨졌어요. 평가치 변동 그래프를 마우스로 드래그할 때 커서가 이상하게 바뀌며 드래그가 끊기던 문제를 고쳤고, 탁월한 수 판정도 다듬었어요 — 내 기물이 걸려 있어도 그와 같거나 더 비싼 상대 기물을 잡는 수라면 사잇수로 보고 희생으로 치지 않아요. 오프닝 이름을 수정하면 그 오프닝에서 만들어진 퍼즐 이름에도 즉시 반영되고, 도감 오프닝 트리에 이론 수를 추가하면 화면에 곧바로 겹치지 않게 다시 그려져요. 게임 리뷰를 닫았다가 같은 수순으로 다시 열어도 진입 애니메이션이 항상 처음부터 다시 재생되고, 체스판 사진 스캔 기능도 다시 정상 동작해요.",
    mascot: {
      intro: { char: "kokoa", expr: "great", name: "KOKOA 코치", align: "left", text: "큰 화면으로 접속하면 게임 리뷰 보드가 훨씬 커져요 — 데스크톱 전용 레이아웃을 만나보세요!" },
      outro: { char: "milku", expr: "happy", name: "MILKU 코치", align: "right", text: "체스판 사진 스캔도 다시 정상으로 돌아왔으니 마음껏 써보세요." },
    },
    highlight: { kind: "icon", Icon: Monitor, color: T.brassHi, label: "게임 리뷰 데스크톱 전용 레이아웃" },
    sections: [
      { cat: "feature", items: [
        "게임 리뷰 화면이 데스크톱에서는 화면 크기에 비례해 커지는 전용 레이아웃으로 바뀌었어요 — 체스보드가 훨씬 커지고, 오른쪽의 평가치 그래프·엔진 라인·기보도 넓은 화면에 맞게 커져요.",
        "리뷰에 들어갈 때 보여주는 분석 대기 화면(체스보드 애니메이션 + 정확도 그래프)도 데스크톱에서 훨씬 넓게 표시돼요.",
        "탁월한 수 판정을 다듬었어요 — 내 기물이 걸려 있어도, 그와 같거나 더 비싼 상대 기물을 잡는 수라면 사잇수(즈반쉬훙)로 보고 희생으로 치지 않아요.",
      ] },
      { cat: "ui", items: [
        "리뷰에 들어갈 때 보여주는 평가치·백/흑 정확도 그래프 세 개가 서로 비슷한 크기로 보이도록 균형을 맞췄어요.",
        "데스크톱 게임 리뷰 화면의 평가치 그래프가 조금 더 작아지고, 코치 설명 말풍선이 보드 옆이 아니라 그 그래프 위쪽으로 옮겨졌어요.",
      ] },
      { cat: "fix", items: [
        "평가치 변동 그래프를 마우스로 드래그할 때 커서가 이상하게(금지 표시로) 바뀌며 드래그가 끊기던 문제를 고쳤어요.",
        "데스크톱 화면에서 게임 리뷰 보드가 항상 작게 고정돼 있던 문제를 고쳤어요 — 이제 화면 크기에 맞게 제대로 커져요.",
