...
const VERSION_HISTORY = [
  {
    ...
    summary: "이 about 페이지 버전 기록에서 빠져 있던 v0.4.8 항목을 채워 넣었어요. 분석 탭에서 자주 두어지는 다음 수의 리체스 통계를 미리 백그라운드에서 당겨와 둬서 통계 표시가 훨씬 빨라졌고, 현재 수 블록·다음 수 블록의 회수(a/b) 표기가 자릿수 때문에 잘려 보이던 문제도 게이지 바를 더 줄여서 고쳤어요(불러오는 동안엔 3-dot bounce 인디케이터가 대신 떠요). 분석 탭 메인 보드가 모바일 화면을 카드 테두리까지 꽉 채우도록 훨씬 커졌고, 그 아래 버튼도 함께 커졌어요. 평가치 막대·엔진 라인이 보이거나 안 보이거나, 엔진 라인이 새로 생기거나 순위가 바뀌거나 — 어떤 경우에도 보드 좌표가 흔들리지 않도록 두 자리 모두 픽셀 단위로 고정했어요. 일일 퍼즐 팝업·퍼즐 풀이 카드에서 기보를 눌러 집중 분석으로 이동했다가 나가면 이제 들어왔던 화면으로 정확히 돌아가고, 퍼즐 카드의 레이팅 배지·테마 장식도 다듬었어요.",
    mascot: {
      intro: { char: "milku", expr: "think", name: "MILKU 코치", align: "left", text: "분석 탭 보드가 모바일 화면을 훨씬 넓게 채워요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "퍼즐 풀다가 기보를 눌러도 다시 그 자리로 돌아와요!" },
    },
    highlight: { kind: "icon", Icon: Zap, color: T.brassHi, label: "분석 탭 보드 확대 + 흔들림 완전 고정 + 퍼즐 화면 복귀 경로 수정 + v0.4.8 버전 기록 보완" },
    sections: [
      { cat: "fix", items: [
      ...
      { cat: "ui", items: [
        ...
        "퍼즐 풀이 카드의 제작자 아이디를 더 크게 표시하고, 눌러서 그 사람의 프로필로 바로 이동할 수 있게 했어요(나오면 풀던 퍼즐로 정확히 돌아와요). PGN/FEN·국면·레이팅 배지와 공유 버튼은 오른쪽으로 정렬했어요.",
