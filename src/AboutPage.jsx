...
const VERSION_HISTORY = [
  {
    ...
    summary: "이 about 페이지 버전 기록에서 빠져 있던 v0.4.8 항목을 채워 넣었어요. 분석 탭에서 자주 두어지는 다음 수의 리체스 통계를 미리 백그라운드에서 당겨와 둬서 통계 표시가 훨씬 빨라졌고, 현재 수 블록·다음 수 블록의 회수(a/b) 표기가 자릿수 때문에 잘려 보이던 문제도 게이지 바를 더 줄여서 고쳤어요(불러오는 동안엔 3-dot bounce 인디케이터가 대신 떠요). 분석 탭 메인 보드·엔진 라인·평가치 막대 영역도 모바일에서 화면 폭에 맞춰 더 크게 표시되고, 요소가 잠깐 사라지거나 수를 둘 때 보드가 흔들리던 문제들을 고쳤어요.",
    mascot: {
      intro: { char: "milku", expr: "think", name: "MILKU 코치", align: "left", text: "분석 탭 보드가 모바일 화면을 훨씬 넓게 채워요!" },
      outro: { char: "kokoa", expr: "wink", name: "KOKOA 코치", align: "right", text: "이제 수를 둬도 보드가 흔들리지 않아요!" },
    },
    highlight: { kind: "icon", Icon: Zap, color: T.brassHi, label: "분석 탭 보드 레이아웃·리체스 통계 로딩 개선 + v0.4.8 버전 기록 보완" },
    sections: [
      { cat: "fix", items: [
        ...
        "분석 탭에서 수를 둘 때, 특히 하단 수 블록에 키워드가 많이 뜨는 포지션일수록 보드 크기가 미세하게 흔들리던 문제를 고쳤어요.",
      ] },
      { cat: "ui", items: [
        "분석 탭 메인 체스보드가 모바일에서 항상 360px로 묶여 있어 화면이 넓은 기기일수록 좌우 여백만 남던 문제를 고쳤어요 — 보드+엔진 라인+평가치 막대 영역이 화면 폭에 맞춰 더 크게 표시돼요(데스크톱의 좌우 2단 레이아웃에서는 기존과 동일하게 360px로 유지돼요).",
      ] },
      { cat: "perf", items: [
