...
const VERSION_HISTORY = [
  {
    ...
    summary: "게임 리뷰가 복잡한 포지션을 더 깊이 분석하도록 시간을 늘려 정확도를 개선했어요. 리뷰에 들어갈 때 보여주는 화면도 완전히 새로워졌어요 — 일러스트가 한 장씩 자연스럽게 넘어가고, 분석이 진행되는 동안 실시간으로 평가 그래프가 그려지며 백·흑 정확도가 계산되는 모습을 확대해서 보여줘요. 정확도 계산 방식 자체도 새로 설계했어요 — 대국이 길어질수록 수 하나의 영향이 자연히 옅어지고, 포지션이 무난했는지 날카로웠는지도 반영돼요. 체스판 사진으로 포지션을 읽는 이미지 스캔의 인식률도 개선했고, 리뷰 화면을 새로고침해도 보던 수에서 이어서 볼 수 있어요. 모바일 리뷰의 엔진 라인·단계별 정확도 말풍선도 더 보기 편해졌고, 라인/퍼즐 클리어 배너는 전용 폰트로 새 단장했어요.",
    mascot: {
    ...
    sections: [
      { cat: "feature", items: [
        "게임 리뷰에 들어갈 때 보여주는 화면이 완전히 새로워졌어요 — 일러스트가 한 장씩 자연스럽게 넘어가고, 분석이 진행되는 동안 실시간으로 평가 그래프가 그려지며 백·흑 각각의 정확도가 계산되는 모습을 확대해서 보여준 뒤 정확도 박스로 자연스럽게 이어져요.",
