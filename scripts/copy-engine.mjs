 ...
 *  (20차 기능2, v0.2.0 기능 추가 → v0.3.5 정리) 두 엔진을 함께 제공한다 — 가볍고 빠른
 *  Stockfish 18 Lite, 그리고 이 둘 중 가장 강력한 Stockfish 17.1(정식 대형 신경망). 게임 리뷰
 *  전용으로 고정해 쓰던 세 번째 엔진 Stockfish 16(NNUE)은 v0.3.5에서 폐기했다 — 게임 리뷰도
 *  이제 이 아래 두 엔진 중 사용자가 설정 탭에서 고른 것을 그대로 쓴다(App.jsx ENGINE_PROFILES 참고).
 *  같은 npm 패키지 이름을 여러 버전으로 동시에 설치할 수 없어 package.json에서
 *  "stockfish171"/"stockfish18" 별칭(npm:stockfish@<버전>)으로 나눠 받는다. */
